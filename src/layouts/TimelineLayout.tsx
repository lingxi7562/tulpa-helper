      ...
      <div className="mx-auto max-w-4xl px-4 py-9 sm:px-8 sm:py-12">
        ...
        <div className="relative ml-2 space-y-5 border-l border-brand-300/80 pl-7 sm:ml-4 sm:pl-10">
          {entries.map((entry, index) => {
            ...
            return <article key={entry.id} className="relative animate-[pageEnter_.45s_var(--ease)_both]" style={{ animationDelay: `${Math.min(index * 45, 360)}ms` }}><span className={`absolute -left-[34px] top-6 h-3.5 w-3.5 rounded-full border-[3px] border-white shadow-[0_0_0_3px_#FDFBF7] sm:-left-[47px] ${stage?.color || 'bg-brand-400'}`} /><Card><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-brand-100 text-base">{icon}</span><div className="min-w-0"><h2 className="truncate text-sm font-black text-brand-900">{entry.title}</h2>{stage && <Badge variant={stageId} className="mt-1 !border-0 !bg-transparent !p-0">{stage.name}</Badge>}</div></div><time className="shrink-0 rounded-full bg-brand-50 px-2.5 py-1 text-[10px] text-brand-400">{entry.created_at?.slice(5, 16)}</time></div>{entry.content && <p className="ml-[52px] mt-3 line-clamp-2 text-sm leading-6 text-brand-600">{entry.content}</p>}{entry.duration_seconds > 0 && <p className="ml-[52px] mt-2 text-[10px] font-bold text-brand-400">⏱ {Math.round(entry.duration_seconds / 60)} 分钟</p>}</Card></article>;
          })}
